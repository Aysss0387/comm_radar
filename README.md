...
网页中的推荐历史保存在 Neon，并按 UTC 日期幂等生成；同一天重复点击“获取今日精读推荐”会直接返回已保存结果。普通本地 CLI 继续使用 `data/daily_feed.jsonl`，自动与周报及历史推荐避重；近两周无合适新文的槽位回落到近四个月并标注「回溯推荐」。
...
~~~bash
# 为一篇论文创建结构化精读卡；已有 My Thoughts 会被���留
...
- 每日推荐、推荐反馈和已采用精读卡持久保存在 Neon；`DATABASE_URL` 由 Vercel 的 Neon 集成注入。未配置数据库的普通本地运行仍回退到 JSON/Markdown 文件。
- 发布环境通过 Zotero Web API 从 Zotero File Storage 临时下载 PDF，解析后立即删除。附件必须是已同步的存储副本；`linked_file`、仅 WebDAV 文件、超过 20 MB 的 PDF 与无可提取文字的扫描件会返回明确提示。
...
论文发现流程只抓取公开元数据、摘要、DOI、期刊页和 OA 链接；精读流程仅使用当前 Zotero API Key 有权读取且已同步到本人 Zotero File Storage 的 PDF，不绕过访问控制，临时文件解析后立即删除。
